...
const ResSaved = ({ place}) => {
  const { currentUser, Res, logout, getPlaces,setRes  } = useAuth();
...
const DeleteTravel= async ()=>{
    try {
      const res = await axios.delete(`https://traveladvisor.herokuapp.com/api/travel/deleteres/${place.idrestuarent}`)
      console.log(res.data)
      swal("Success",res.data,"success")
      
     let newPlace = Res.filter((r)=>r.idrestuarent !== place.idrestuarent)
     setRes(newPlace)
  } catch (error) {
